...
function modbus_crc16(data, offset, size) {
    return _crc(data, offset, size, 0xffff, crc_table);
}

function _crc(data, offset, size, initValue, table) {
    var _crc = initValue;
    for (var i = 0; i < size; i++) {
        var tmp = (_crc ^ data[i + offset]) & 0xff;
        _crc >>= 8;
        _crc ^= table[tmp];
        _crc &= 0xffff;
    }
